...
% title('Figure 6: The beamforming results using the optimized measurement matrix.');
...
%--------------------------------L1_norm----------------------------------------------------------------%
% generate matrix D
ang_range = linspace(-90, 90, 1800);  % the angle range
D = exp(1j * 2 * pi * param.d_E * (0:param.M-1).' * sind(ang_range));  % Fourier dictionary
% Interference Vector
...
%--------------------------------------figure2-----------------------------------------------------------%
figure;
plot(param.cont_ang, sp_tmp, 'LineWidth', 2);
legend_str{1} = 'Proposed method';
sp_tmp2 = 20*log10(abs(z));
sp_tmp2 = sp_tmp2 - max(sp_tmp2);

hold on; 
plot(fft_angle_vector, 10*log10(fft_power_spectrum), 'LineWidth', 1);
legend_str{2} = 'FFT';

hold on; stem(param.theta_TR, sp_tmp2, 'BaseValue', min(sp_tmp)-10, 'LineWidth', 2, 'MarkerSize',8,'Marker', 'o');
legend_str{3} = 'Ground-truth DOA';

hold on;
plot(ang_range, 10 * log10(abs(x_hat) / max(abs(x_hat))),'-.', 'LineWidth', 1);
legend_str{4} = 'L1-norm';
...
% title('Figure 4: The estimated spatial spectrum with SNR being 20 dB');
grid on;
