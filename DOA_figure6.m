...
SNR_dB = 20;
param.theta_AR = -10 + rand(1);
param.Ns = 8:9:62;  % the range of the number of measurements

rmse_fft = zeros(1, length(param.Ns));
rmse_proposed = zeros(1, length(param.Ns));
rmse_crlb = zeros(1, length(param.Ns));
rmse_l1norm = zeros(1, length(param.Ns));

for i = 1:length(param.Ns)
    ...
    disp("rmse_proposed(i):" +rmse_proposed(i));

    rmse_crlb(i) = calculate_crlb(z, r_z, param, noise_pow);

    %--------------------------L1_norm-------------------------------------------------%
    ang_range = linspace(-90, 90, 360); 
...
plot_results(param.Ns, rmse_fft, rmse_proposed, rmse_l1norm, rmse_crlb);
...
function plot_results(Ns, rmse_fft, rmse_proposed, rmse_l1norm, rmse_crlb)
    figure;
    semilogy(Ns, rmse_fft, '-bs', 'DisplayName', 'FFT','LineWidth',2,'MarkerFaceColor', 'b', 'MarkerSize', 6);
    hold on;
    semilogy(Ns, rmse_proposed, '-ro', 'DisplayName', 'Proposed Method','LineWidth',2,'MarkerFaceColor', 'r', 'MarkerSize', 6);
    semilogy(Ns, rmse_l1norm, '-+', 'DisplayName', 'L1-norm','LineWidth',2,'MarkerFaceColor', 'y', 'MarkerSize', 6);
    semilogy(Ns, rmse_crlb, '-g', 'DisplayName', 'CRLB','LineWidth',2,'MarkerFaceColor', 'g', 'MarkerSize', 6);
    xlabel('The number of measurements');
    ylabel('RMSE (deg)');
    legend show;
    grid on;
    % title('Fig.9.The estimation performance with different numbers of RIS elements');
    set(gca, 'YScale', 'log');
    ylim([1e-2 10]); 
    xlim([8 64]);
end
