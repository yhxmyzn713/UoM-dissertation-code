...
G_tilde = randn(param.M, param.M) + 1j*randn(param.M, param.M);
...
% r_q = param.G * param.get_steer(param.theta_AR, param.M) * q;  put it into the loop
...
function plot_results(SNRs, rmse_fft, rmse_proposed, rmse_l1norm, rmse_crlb)
    ...
    legend show;
    grid on;
    % title('Fig.5. The estimated performance with different SNRs (without mmo).');
    set(gca, 'YScale', 'log');  
    ylim([1e-3 10]);  
end
